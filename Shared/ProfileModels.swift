...
/// 片方だけに候補が増えて体験がずれることを防ぐため、共有モデル側で一元管理する。
...
/// windowLayoutアクションで選べるウィンドウ配置。
/// iPad/Macの編集画面とMacの実行側（WindowLayoutManager）が同じ定義を参照するよう、
/// 共有モデル側で一元管理する
enum WindowLayoutPreset: String, CaseIterable, Identifiable, Codable {
  ...
  var systemImage: String {
    ...
    case .threeSplit: return "rectangle.split.3x1.fill"
    }
  }
}
